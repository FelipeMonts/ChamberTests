...
### use rbind to attache the new data to the data frame
...
# Select the columns for the splitting and the analysis and collect them in the file chamber.1
...
polygon(c(rev(pred.x), pred.x), c(rev(conf_interval[,3]),conf_interval[,2]), col="lightblue" )
points(spl[[1]]$Time, spl[[1]]$Concentration, col="RED", pch=16, cex=2)
...
#for that remember that coefs is a named vector two rows ("(Intercept)" "Time") and 40 columns
...
# lets creat a data frame with the data in t(coefs) and the names of t(coefs) obtained  with dimnanes ())
...
# create the variable "Test.Plot.Chamber" in the coefs.otherdata data frame
